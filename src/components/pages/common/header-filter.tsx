...
export const DEFAULT_TRACK_COLUMNS: TrackColumnKey[] =
  TRACK_COLUMNS.map(({ key }) => key)
...
export const getActiveTrackColumns = (
  ...
  return TRACK_COLUMNS.filter(
    ({ key }) =>
      REQUIRED_TRACK_COLUMNS.includes(key) ||
      visible.has(key)
  )
}
...
export const useTrackColumns = () => {
  ...
  useEffect(() => {
    try {
      ...
      const validKeys = new Set<TrackColumnKey>(
        DEFAULT_TRACK_COLUMNS
      )
      ...
      const savedColumns = parsed.filter(
        (key): key is TrackColumnKey =>
          typeof key === "string" &&
          validKeys.has(key as TrackColumnKey)
      )
    ...
    setVisibleColumns((current) =>
      current.includes(key)
        ...
        : DEFAULT_TRACK_COLUMNS.filter(
            (column) =>
              current.includes(column) || column === key
          )
    )
  }

  const resetColumns = () => {
    setVisibleColumns(DEFAULT_TRACK_COLUMNS)
  }
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <button
          ...
          className="
            ...
            border-white/10 bg-white/[0.04] px-3 py-2
      ...
      <PopoverContent
        align="end"
        className="
          w-52 border-white/10 bg-zinc-950
          p-2 text-zinc-300 shadow-2xl
        "
      >
        <p
          className="
            px-2 pb-2 text-[9px] font-semibold uppercase
            tracking-[0.14em] text-zinc-600
          "
        >
          Show columns
        </p>

        {TRACK_COLUMNS.filter(({ label }) => label).map(
          (column) => {
            const isLocked =
        ...
        <button
          ...
          onClick={onReset}
          className="
            mt-2 w-full rounded-lg border border-white/10
            px-2 py-2 text-xs text-zinc-400 transition
